    ...
    def _render_content(self):
        ...
        if self.articles:
            ...
            for article in self.articles:
                ...
                if isinstance(article_num, str) and ('第' in article_num and '条' in article_num):
                    # 已经是"第X条"格式，直接使用
                    article_num_display = article_num
                else:
                    # 纯数字，包装成"第X条"
                    article_num_display = f"第{article_num}条"
                
                # 显示条文
                html_parts.append(f'''
                <div class="article" id="article-{article.article_num}">
                    <span class="article-num">{article_num_display}</span> 
