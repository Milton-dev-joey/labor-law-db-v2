...
from src.case_workbench import CaseWorkbenchWidget
...
class MainWindow(QMainWindow):
    ...
    def _create_ui(self):
        ...
        # 5. 案件工作台
        self.case_workbench = CaseWorkbenchWidget(self.db)
        self.left_stack.addWidget(self.case_workbench)
    ...
    def _create_menu(self):
        ...
        dashboard_action = QAction("🏠 返回法律库", self)
    ...
    def _create_toolbar(self):
        ...
        # 返回法律库按钮
        home_btn = QPushButton("🏠 法律库")
        home_btn.setToolTip("返回法律库 (Ctrl+H)")
        ...
        # 案件工作台按钮（放在最右边）
        workbench_btn = QPushButton("⚖️ 案件工作台")
        ...
        """)
        workbench_btn.clicked.connect(self._show_case_workbench)
        toolbar.addWidget(workbench_btn)
        ...
        # 回到法律库按钮（仅在非法律库时显示）
        self.back_btn = QPushButton("← 返回法律库")
    ...
    def _show_fullscreen_dashboard(self):
        ...
        if hasattr(self, 'statusbar') and self.statusbar:
            self.statusbar.showMessage("法律库 - 按位阶浏览法律，点击卡片查看详情")
    ...
    def _show_case_workbench(self):
        """显示案件工作台"""
        self.is_fullscreen_dashboard = True
        self.right_widget.setVisible(False)
        self.left_stack.setCurrentIndex(4)
        if hasattr(self, 'case_workbench'):
            self.case_workbench.refresh()
        self.main_splitter.setSizes([self.main_splitter.width(), 0])
        if hasattr(self, 'statusbar') and self.statusbar:
            self.statusbar.showMessage("案件工作台 - 管理案件和生成文书")
    ...
    def _show_about(self):
        ...
        QMessageBox.about(
            ...
            <p><b>操作提示：</b></p>
            <ul>
                <li>法律库点击法律卡片进入阅读模式</li>
                <li>选中条文文字可添加精准批注</li>
                <li>Ctrl+H 快速返回法律库</li>
            </ul>
            """
        )
